...
$value = $subjectID = $userID = $username = $id = "";
if(isset($_POST['value'])){
    ...
    $sql = $conn->prepare("UPDATE comments SET content = ? WHERE id = ?");
    $sql->bind_param("si", $value, $id);
    $sql->execute();
    makeTable();
    echo  '<textarea rows="8" cols="20" style="resize:none;margin-left: 12%" id="changeComment" name="changeComment" hidden></textarea>';
    echo '<button id="newComment" onclick="update_comments();changeVisibility()" hidden>Izmijeni</button>';
